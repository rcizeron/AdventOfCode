fun day1result() = "sum: " + input.map { get1stAndLastDigitFromAlphanumericalString(it) }.sum()
...
private fun get1stAndLastDigitFromAlphanumericalString(line: String): Int {

    var newLine = line
    dictionary.map { (key, value) -> newLine = newLine.replace(key, value) }


    val firstDigit = newLine.first { it.isDigit() }
    val lastDigit = newLine.last { it.isDigit() }
